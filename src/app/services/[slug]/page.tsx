...
import { affiliateServiceAlternates, financialReviewServiceAlternates, musicServiceAlternates, robloxServiceAlternates } from "@/i18n/locales";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (service.slug === "roblox-brand-experiences") {
    return { ...metadata, alternates: { ...metadata.alternates, languages: robloxServiceAlternates } };
  }
  if (service.slug === "music-distribution-artist-digital-presence") {
    return { ...metadata, alternates: { ...metadata.alternates, languages: musicServiceAlternates } };
  }
  return metadata;
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isRobloxExperienceService = service.slug === "roblox-brand-experiences";
  const isMusicDistributionService = service.slug === "music-distribution-artist-digital-presence";
  ...
  const serviceSchema = {
    ...
    ...(isAutonomousSeoService
      ...
      : isMusicDistributionService
        ? { serviceType: "Music distribution preparation, artist digital presence and release management", areaServed: "Worldwide" }
  ...
  return (
    <>
      ...
      <Section eyebrow={service.eyebrow} title={service.heroTitle || service.title} titleAs="h1">
        ...
        {isBusinessSystemsChatGptService ? (
          <div className="mt-6 flex flex-wrap gap-3">
            <a className="btn-primary" href={site.whatsappHref} rel="noopener noreferrer" target="_blank">
              שלחו לנו אילו מערכות יש לכם
            </a>
            <a className="btn-secondary" href="https://talktodata.navines.com" rel="noopener noreferrer" target="_blank">
              לצפייה ב־TalkToData
            </a>
        ...
        {isMusicDistributionService ? (
          <div className="mt-6 flex flex-wrap gap-3">
            <a className="btn-primary" href={`${site.whatsappHref}?text=${encodeURIComponent("שלום, אני אמן או מוזיקאי ורוצה לבדוק הפצה וניהול נוכחות דיגיטלית.")}`} rel="noopener noreferrer" target="_blank">
      ...
      <Section>
        <div className="grid gap-5">
          ...
          {isAmazonSeoWebsiteService ? (
            <article className="rounded-premium border border-purple-200/15 bg-purple-500/[0.06] p-5">
              <h2 className="text-2xl font-semibold text-white">חשוב לדעת על SEO מחוץ ל אמזון</h2>
              <p className="mt-3 text-lg leading-8 text-zinc-300">
                אנחנו לא מבטיחים מקום ראשון, אינדוקס יומי או מכירות מובטחות. העבודה מתמקדת בבניית אתר שימושי, תוכן מקורי, מבנה בינלאומי נכון, מדידה והפניות מסודרות לעמודי אמזון. לא מעתיקים תיאורי מוצר באופן עיוור ולא מבצעים scraping בניגוד לתנאי שימוש.
              </p>
            </article>
          ) : null}
            ...
            <article className="border-t py-6" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {service.serviceHighlights.map((highlight) => {
                  ...
                  const primaryHref = highlight.href || highlight.externalHref;
                  ...
                  return primaryHref ? (
                    <div className="border-t py-4" key={highlight.title} style={{ borderColor: "var(--border)" }}>
                      {isExternalPrimary ? (
                        ...
                        <Link className="block transition hover:text-sky-200" href={primaryHref}>{content}</Link>
                      )}
...
function GlobalBrandServiceContent({ service }: { service: (typeof services)[number] }) {
  const audiences = [
    ...
    {
      title: "מותג שמוכר דרך אמזון, חנויות או מפיצים",
