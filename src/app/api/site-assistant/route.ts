...
const amazonKeys = ["אמזון", "amazon", "איביי", "ebay", "seller central", "account health", "asin", "listing", "fba", "poa", "השעיה באמזון", "חשבון אמזון", "מוכר באמזון", "מוכרים באמזון"];
...
const topics: Topic[] = [
  {
    keys: ["קידום ב chatgpt", "קידום ב־chatgpt", "קידום במנועי ai", "מנועי ai", "geo", "aeo", "perplexity", "פרפלקסיטי", "gemini", "ג׳מיני", "claude", "קלוד", "llms.txt", "נראות ב ai"],
    answer:
      "אפשר לחזק את הנראות של אתר ב־ChatGPT, Gemini, Claude, Perplexity ומנועי תשובות נוספים באמצעות גישה תקינה לזחלנים, ישות חברה עקבית, Schema, עמודי מקור, תוכן שניתן להבין ולצטט וקישורים פנימיים. אין הבטחה לאזכור או המלצה של מודל, ו־llms.txt הוא כלי ניווט משלים בלבד.",
    href: "/services/ai-search-visibility-geo",
    label: "לקידום במנועי AI ו־ChatGPT",
  },
  {
    keys: ["search console", "גוגל קונסול", "google analytics", "גוגל אנליטיקס", "סוכן seo", "seo אוטונומי", "קידום אורגני אוטונומי", "בוט seo", "לוגים", "דיפלוי"],
    answer:
      "אפשר לחבר Google Search Console, גוגל אנליטיקס, סריקות, קוד ולוגים לסוכן SEO ממוקד. הוא מזהה הזדמנויות ותקלות, מכין שינוי, מריץ בדיקות, פורס Preview או שינוי מאושר ומודד שוב. מתחילים בקריאה בלבד ומגדירים במפורש אילו שינויים או דיפלויים רשאים לרוץ אוטומטית.",
    href: "/services/autonomous-seo-agent-search-console-chatgpt",
    label: "לעמוד סוכן SEO אוטונומי",
  },
  {
    keys: ["connector", "קונקטור", "api", "איי פי איי", "אין api", "בלי api", "אין איי פי איי", "webhook", "sumit", "סמיט", "סמית", "סאמיט", "ריווחית", "שמאית", "מוסכית", "נשר"],
    answer:
      "יש API רשמי? מתחברים אליו. אין API מוכן? בודקים Webhook, ייצוא קבצים, מסד נתונים, Plugin או Connector מורשה אחר. NAVINES Bridge היא שכבת החיבור המותאמת שלנו ל־ChatGPT ולצ׳ט ג׳י פי טי, כולל בדיקת היתכנות ל־SUMIT/סמיט/סמית, ריווחית, שמאית, מוסכית 2020, נשר ומערכות נוספות, בלי לעקוף הרשאות או להבטיח חיבור לפני בדיקה.",
    href: "/services/api-integrations",
    label: "לפיתוח Connector ו־API מותאם",
  },
  {
    keys: ["חשבונית אונליין", "חשבונית און ליין", "morning", "מורנינג", "green invoice", "גרין אינוויס", "חשבונית ירוקה", "priority", "פריוריטי", "קו מערכות"],
    answer:
      "אפשר לבדוק חיבור של חשבונית אונליין, Morning/מורנינג, Green Invoice/חשבונית ירוקה, Priority/פריוריטי, קו מערכות ומערכות עסקיות נוספות אל ChatGPT וצ׳ט ג׳י פי טי. כאשר קיימים API, Webhook, ייצוא, מסד נתונים או גישה מורשית אחרת, אנחנו ממפים את הנתונים וההרשאות ובונים שיחה, ניתוח ואוטומציה על נתוני אמת.",
    href: "/services/business-systems-chatgpt-integration",
    label: "לחיבור חשבונית אונליין ומערכות",
  },
  {
    keys: ["סוכן ai", "סוכני ai", "סוכן בינה", "סוכני בינה", "ai agent", "ai agents", "סוכן אוטונומי", "אוטונומי", "מפעל", "מפעלים", "מוסך", "מוסכים", "מוסכית", "שמאי", "שמאית"],
    answer:
      "אנחנו בונים סוכני AI וסוכני צ׳ט ג׳י פי טי ממוקדי מטרה לעבודה שוטפת: סוכן לנתונים, תפעול, רואי חשבון, מפעלים, מוסכים, שמאים, שירות, מכירות או תהליך פנימי. מגדירים תפקיד, מקורות, כלים, הרשאות ולוגים; עבודה אוטונומית מתאפשרת רק בגבולות שנבדקו ועם אישור אנושי לפעולות רגישות.",
    href: "/services/chatgpt-ai-agents-business",
    label: "לעמוד סוכני AI לעסקים",
  },
  {
...
function findTopic(question: string, history: HistoryMessage[] = []) {
  ...
  for (const item of [...history].filter((item) => item.role === "user").reverse()) {
...
function detectIntent(question: string, history: HistoryMessage[] = []): Intent {
  const previousUserText = history
    ...
    .map((item) => item.text)
    .join(" ");
  const fullText = `${previousUserText} ${question}`;
  ...
  const currentAmazon = hasAny(question, amazonKeys);
  const contextAmazon = hasAny(previousUserText, amazonKeys);
  ...
  if (currentAmazon || (followUp && contextAmazon)) {
    return "amazon_issue";
  }

  if (hasAny(question, aiChatKeys) || (followUp && hasAny(previousUserText, aiChatKeys))) {
    return "ai_chat_interest";
  }

  if (hasAny(question, courseKeys) || (followUp && hasAny(previousUserText, courseKeys))) {
...
function topicForIntent(intent: Intent, question: string, history: HistoryMessage[]): Topic | undefined {
  ...
  if (intent === "ai_chat_interest") {
    return findTopicFromText(question) || findTopicByHref("/services/ai-chat-for-websites");
  }
...
function fallbackPayload(question: string, history: HistoryMessage[]) {
  ...
  let answer =
    topic?.answer ||
    "אנחנו עוזרים לעסקים עם אתרים, AI, אוטומציה, תמיכה טכנית, קורסים וכלים חכמים. אם תגידו לי מה אתם מנסים לעשות או מה הבעיה, אכוון אתכם לעמוד או לשירות המתאים.";
...
להבנת נושא השיחה השתמש רק במה שהמשתמש כתב ובהודעות user קודמות. אל תסיק נושא מתוך רשימת שירותים או תשובה שהעוזר עצמו נתן קודם.
השתמש בהיסטוריית המשתמש כדי להבין שאלות המשך כמו "כמה זה עולה", "אפשר כזה גם לאמזון", "ומה עם זה" או "שלח אותי לעמוד".
אל תקפוץ ל-Amazon אלא אם המשתמש עצמו הזכיר Amazon, אמזון, Seller Central, Account Health, ASIN, listing, FBA, POA או שהודעת user קודמת היתה על Amazon בצורה ברורה.
אם לא בטוח מה הנושא, שאל שאלה קצרה אחת להבהרה במקום להפנות לעמוד לא בטוח.
...
אם זה קורסים, Amazon, צ׳ט AI לאתרים, סוכן SEO אוטונומי, Search Console, חשבונית אונליין ומערכות עסקיות ל־ChatGPT, פיתוח Connector או API מותאם, SUMIT, שמאית, מוסכית, סוכני AI לעסקים, TalkToData או תמיכה טכנית, הפנה לעמוד המתאים.
