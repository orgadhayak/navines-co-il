...
import type { LocalizedRobloxContent } from "@/content/localized/roblox-brand-experiences";
import { localizedRobloxArticlePaths, localizedRobloxServicePaths, siteLocales } from "@/i18n/locales";
import { site } from "@/data/site";

const robloxWorldUrl = "https://www.roblox.com/games/8820246222/NAVINES-WORLD";

export function LocalizedRobloxExperience({ content }: { content: LocalizedRobloxContent }) {
  const locale = siteLocales[content.locale];
  const path = localizedRobloxServicePaths[content.locale];
  ...
  const serviceSchema = {
    ...
    serviceType: "Roblox game development and interactive brand experiences",
        ...
        <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:grid lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-14 lg:px-8 lg:py-20">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <a className="btn-secondary" href={robloxWorldUrl} rel="noopener noreferrer" target="_blank">{content.service.gameLabel}</a>
            </div>
          </div>
          <aside className="mt-10 border-y py-6 lg:mt-0" style={{ borderColor: "var(--border)" }}>
            <p className="text-sm font-semibold text-glowred">NAVINES WORLD</p>
            <h2 className="mt-3 text-2xl font-semibold">{content.service.gameTitle}</h2>
            <p className="mt-3 text-base leading-7" style={{ color: "var(--text-muted)" }}>{content.service.gameText}</p>
            <a className="editorial-link mt-5 inline-flex" href={robloxWorldUrl} rel="noopener noreferrer" target="_blank">{content.service.gameLabel}</a>
          </aside>
        </section>

        <LocalizedSection title={content.service.buildTitle}>
          <div className="grid gap-x-9 md:grid-cols-2">
            {content.service.build.map((item) => <p className="border-t py-5 text-lg leading-8" key={item} style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>{item}</p>)}
          </div>
        </LocalizedSection>
        ...
        <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
          <div className="border-y py-8 md:flex md:items-end md:justify-between md:gap-10" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="mt-6 flex flex-wrap gap-3 md:mt-0">
              <a className="btn-primary" href={whatsappHref} rel="noopener noreferrer" target="_blank">{content.service.ctaLabel}</a>
              <Link className="btn-secondary" href={localizedRobloxArticlePaths[content.locale]}>{content.service.articleLabel}</Link>
            </div>
          </div>
        </section>
      </main>
