...
import type { LocalizedFinancialReviewContent } from "@/content/localized/payment-discrepancy-review";
import { site } from "@/data/site";
import { localizedFinancialReviewArticlePaths, siteLocales } from "@/i18n/locales";

export function LocalizedFinancialReview({ content }: { content: LocalizedFinancialReviewContent }) {
  const { locale, service } = content;
  const localeMeta = siteLocales[locale];
  const servicePath = `/${locale}/services/payment-discrepancy-review`;
  const articlePath = localizedFinancialReviewArticlePaths[locale];
  ...
  return (
    <main>
      ...
      <ContentSection title={service.scopeTitle}>
      ...
      <ContentSection title={service.safeguardsTitle}>
        <div className="max-w-4xl divide-y" style={{ borderColor: "var(--border)" }}>
          {service.safeguards.map((item) => <p className="py-5 text-lg leading-8" key={item} style={{ color: "var(--text-muted)" }}>{item}</p>)}
        </div>
      </ContentSection>
...
const whatsappMessages = {
  de: "Hallo Navines, ich möchte prüfen, ob sich eine strukturierte Prüfung von Zahlungen oder möglichen Erstattungen lohnt.",
  jp: "Navines、支払いまたは返金の可能性について、整理した確認が必要か相談したいです。",
  ar: "مرحباً Navines، أريد معرفة ما إذا كانت مراجعة منظمة للمدفوعات أو الاستردادات المحتملة مناسبة.",
  hi: "नमस्ते Navines, मैं जानना चाहता/चाहती हूँ कि भुगतान या संभावित रिफंड की संरचित समीक्षा उपयोगी होगी या नहीं।",
  fr: "Bonjour Navines, je souhaite savoir si une vérification structurée de paiements ou de remboursements possibles est pertinente.",
  zh: "您好 Navines，我想咨询是否适合对付款或可能的退款进行结构化核查。",
};
